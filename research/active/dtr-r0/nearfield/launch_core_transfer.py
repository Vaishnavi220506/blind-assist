...
def main():
    ...
    data=json.loads(spec.read_text());assert len(data['cases'])==432
    ...
    capture_script=Path(__file__).with_name('core_transfer_capture.py')
    ...
    try:
        code=tree.wait(timeout=3600)
        receipt=json.loads((out/'receipt.json').read_text())
        assert code==0 and receipt['status']=='PASS' and receipt['frame_count']==432
        ...
        assert len(receipt['view_readiness'])==432 and all(r['status']=='READY' for r in receipt['view_readiness'])
