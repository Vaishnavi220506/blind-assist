"""Unreal-only96-frame capture. No map save, inference, labels in observations.
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==96 and len(geometry)==96
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==96
            assert len({c['name'] for c in cases})==96 and len({c['clip_id'] for c in cases})==8
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=96,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=96))
            if index==len(cases):
