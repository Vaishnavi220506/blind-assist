"""Launch the one frozen 48-clip/576-frame local-transfer UE capture.
...
MANDATORY_CODE = {
    'local_transfer_capture.py', 'launch_local_transfer.py', 'local_transfer_spec.py',
    'ue_capture_readiness.py', 'core_transfer_spec.py', 'ba_camera_corridor_spec.py',
    'spatial_bce_spec.py', 'data_coverage_spec.py', 'query_occupancy_spec.py',
    'LOCAL_TRANSFER_PROTOCOL_20260922.md',
}
MANDATORY_INPUTS = {
...
def main():
    ...
    capture_script = Path(__file__).with_name('local_transfer_capture.py').resolve()
    ...
    env = dict(os.environ, BA_LOCAL_TRANSFER_SPEC=str(spec), BA_LOCAL_TRANSFER_OUTPUT=str(out),
        BA_LOCAL_TRANSFER_SCRIPT=str(capture_script), BA_LOCAL_TRANSFER_PROTOCOL_SHA=source['protocol_sha256'],
        BA_LOCAL_TRANSFER_SPEC_SHA=source['spec_sha256'], BA_LOCAL_TRANSFER_SCRIPT_SHA=source['capture_script_sha256'])
