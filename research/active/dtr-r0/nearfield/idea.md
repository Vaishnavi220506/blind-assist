...
2026-09-19 文献复核：[NFO 失败后的研究重估](NFO_RESEARCH_REASSESSMENT_20260919.md)。
结合 ZCR、32 帧迁移和完整训练集负结果，重新区分空间定位、绝对距离与区域
回波归属。保留“前后景分层与区域回波归属”为待验证假设；优先建议一次冻结的
空间／距离能力对照，有真实数据条件时先检查多目标／CNH 是否保留近层。
不把分布／直方图论文当作当前单 dominant distance 的等价证据，也不重开旧阈值
或 loss 配方。本轮仅资料研究，没有新实验；下文 9 月 10 日记录保留原范围。

2026-09-10。以下文献判断先于 MZ5 实验，保留原始假设与比较顺序。
文献阶段没有新训练、采集或实验结果；MZ0 的组件角色、MZ1 的 challenger 角色
以及 MZ3/MZ4 的已完成状态保持原有含义。
以下建议来自原始论文、作者项目/代码说明、ST 文档和本地实现的交叉核对。

同日执行追记：前两项比较已完成。[固定输出融合](MZ5_FIXED_ENSEMBLE_RESULTS_20260910.md)
达到1378/1500，已导出132872参数的等价紧凑读出；[局部融合](MZ5_SPATIAL_FUSION_RESULTS_20260910.md)
比匹配的提前平均对照仅多对1/1500，HEAD_ONLY near多对3/150，未超过固定融合。
因此下一轮受控实验采用固定融合基线，并保留远距离召回下降这一取舍；局部机制
暂留作对照。时序建议仍是待检验想法。本次执行结果由[当前路线](../CURRENT.md)管理。
