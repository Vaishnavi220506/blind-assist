"""Unreal-only1728-frame query-occupancy capture (144 clips, 12 posed frames each).
...
OUT = Path(os.environ['BA_QUERY_OCCUPANCY_OUTPUT'])
SPEC = Path(os.environ['BA_QUERY_OCCUPANCY_SPEC'])
SCRIPT = Path(os.environ['BA_QUERY_OCCUPANCY_SCRIPT'])
PROTOCOL_SHA = os.environ['BA_QUERY_OCCUPANCY_PROTOCOL_SHA']
SPEC_SHA = os.environ['BA_QUERY_OCCUPANCY_SPEC_SHA']
SCRIPT_SHA = os.environ['BA_QUERY_OCCUPANCY_SCRIPT_SHA']
...
def component(source, fmt):
    ...
    actor.set_actor_label('BA query occupancy capture')
...
def prepare(case):
    if not frames or frames[-1]['clip_id'] != case['clip_id']:
        ...
        for obj in case['objects']:
            ...
            actor.set_actor_label('BA query occupancy '+case['clip_id']+' '+obj['name'])
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==1728 and len(geometry)==1728
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==1728
            assert len({c['name'] for c in cases})==1728 and len({c['clip_id'] for c in cases})==144
            for offset in range(0, 1728, 12):
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=1728,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=1728))
            if index==len(cases):
                fx=640/(2*math.tan(math.radians(50)))
                write(OUT/'observations/manifest.json',dict(schema='ba-query-occupancy-rgb-v1',
