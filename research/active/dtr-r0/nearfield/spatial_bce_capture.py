"""Unreal-only2880-frame complete-event capture (120 clips, 24 frames each). No map save, inference, labels in observations.
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==2880 and len(geometry)==2880
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==2880
            assert len({c['name'] for c in cases})==2880 and len({c['clip_id'] for c in cases})==120
            for offset in range(0, 2880, 24):
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=2880,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=2880))
            if index==len(cases):
