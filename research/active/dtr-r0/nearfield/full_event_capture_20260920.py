"""Unreal-only576-frame complete-event capture (24 clips, 24 frames each). No map save, inference, labels in observations.
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert len({c['name'] for c in cases})==576 and len({c['clip_id'] for c in cases})==24
            for offset in range(0, 576, 24):
