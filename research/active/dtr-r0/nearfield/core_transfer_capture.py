"""Unreal-only432-frame capture. No map save, inference, labels in observations.
...
def authenticate(case):
    ...
    floor_hit = u.SystemLibrary.line_trace_single(world,u.Vector(pose['x']*100,0,80),
        u.Vector(pose['x']*100,0,-100),u.TraceTypeQuery.TRACE_TYPE_QUERY1,True,[],u.DrawDebugTrace.NONE)
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==432 and len(geometry)==432
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==432
            assert len({c['name'] for c in cases})==432 and len({c['clip_id'] for c in cases})==36
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=432,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=432))
            if index==len(cases):
