"""Launch the one frozen 144-clip/1728-frame query-occupancy UE capture.
...
EXPECTED_FRAMES = 1728
EXPECTED_CLIPS = 144
FRAMES_PER_CLIP = 12
CAPTURE_BUDGET_SECONDS = 2400
EXPECTED_MAP_SHA = 'cf35e5c9df54cd0f781f09ea8105fe8ef6078ed0822d4e594d64216e79a254fb'
MANDATORY_CODE = {
    'query_occupancy_capture.py', 'launch_query_occupancy.py', 'query_occupancy_spec.py',
    'ue_capture_readiness.py', 'core_transfer_spec.py', 'ba_camera_corridor_spec.py',
    'spatial_bce_spec.py', 'data_coverage_spec.py',
}
MANDATORY_INPUTS = {
...
def validate_cases(data):
    from query_occupancy_spec import check_spec
    from data_coverage_spec import specification as old_coverage
    check_spec(data, [old_coverage()])
    cases = data['cases']
...
def main():
    ...
    capture_script = Path(__file__).with_name('query_occupancy_capture.py').resolve()
    ...
    env = dict(os.environ, BA_QUERY_OCCUPANCY_SPEC=str(spec), BA_QUERY_OCCUPANCY_OUTPUT=str(out),
        BA_QUERY_OCCUPANCY_SCRIPT=str(capture_script), BA_QUERY_OCCUPANCY_PROTOCOL_SHA=source['protocol_sha256'],
        BA_QUERY_OCCUPANCY_SPEC_SHA=source['spec_sha256'], BA_QUERY_OCCUPANCY_SCRIPT_SHA=source['capture_script_sha256'])
