...
SEED = 179017
DISTANCES = (3.3, 3.6, 3.8, 4.0, 4.2, 4.5)
...
def source():
    ...
    spec = json.loads(json.dumps(spec).replace('mz136_', 's1confirm_'))
    spec['schema'] = 's1-targeted-frozen-confirmation-v1'
    ...
    for index, group in enumerate(spec['scene_groups']):
        ...
        for f in group_frames:
            target = f['objects'][0]
            if fi == 2:
                target['size_m'][1] = (.020, .032, .044, .056, .068, .080)[k]
