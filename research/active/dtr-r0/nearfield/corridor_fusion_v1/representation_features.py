"""Frozen MZ143 frontend copy for a representation-only Development ablation.

The original module stays unchanged because prior experiment seals bind its bytes.
``multi`` must reproduce every original feature exactly; ``single`` only restricts
the candidate iterator. No outcome, range or corridor score selects a candidate.
...
def select_descriptors(obs, descriptors, association):
    if association == 'multi':
        return descriptors
    if association != 'single':
        raise ValueError('Expected multi or single association')
    eligible = []
    for desc in descriptors:
        overlap = intersection(obs['box'], desc['box'])
        if overlap is not None:
            shared = area(overlap)
            iou = shared / (area(obs['box']) + area(desc['box']) - shared)
            eligible.append((-iou, tuple(desc['box']), desc))
    return [min(eligible, key=lambda x: x[:2])[2]] if eligible else []


def raw_indices(names):
    keep_geometry = {'rgb.mask_fraction', 'rgb.foreground_boxes',
        'slots.usable_valid', 'slots.merged', 'slots.outside_or_partial_rgb',
        'rgb.positive_gradient_mean', 'rgb.negative_gradient_mean'}
    return np.array([i for i, name in enumerate(names)
        if i < 2217 or name in keep_geometry], int)


def extract(row, image, yaw, association='multi'):
    ...
    for obs in observations:
        for desc in select_descriptors(obs, descriptors, association):
            overlap = intersection(obs['box'], desc['box'])
            if overlap is None:
                continue
            l, t, r, b = desc['box']
    ...
    audit = dict(schema=METHOD['schema'], association=association, public_slots=len(audit_slots), encoded_slots=128,
