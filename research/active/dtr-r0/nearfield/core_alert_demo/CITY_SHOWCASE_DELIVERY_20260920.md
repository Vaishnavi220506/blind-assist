...
原1080p PNG平均约4.15 MB。旧实现每帧更换 `src` 前隐藏图像并显示同步提示；预加载没有等待解码，也没有保留解码后的节点。现在用最多32张图片的缓存，三个预取任务准备当前片段与下一段开头，直接切换已经解码的原图节点。图像就绪后才同时提交帧号、几何与告警数据；慢读保留上一完整帧，180 ms后才提示。暂停和后续跳转使旧请求失效，缺图仍明确报告。跨集合等待时锁定逐帧控件，避免对上一集合操作。默认1×，0.5×仍可选。

`test_replay_images.cjs`覆盖解码复用、就绪帧无加载提示、异步提交、快速跳转竞态、暂停取消、失败重试、缓存上限和预取取消。浏览器2×完整八段连播，逐次检查全部192帧图像与标签同步，加载提示/空白/错位均0；旧保持导览、快速首尾跳转和集合切换已复核。原1296帧及新增192帧的显示契约与27组指标测试通过。检查针对本机本地HTTP播放，不承诺任意设备的解码速度；0.2秒原采样间隔与所有图像、观测及预测保持原样。
