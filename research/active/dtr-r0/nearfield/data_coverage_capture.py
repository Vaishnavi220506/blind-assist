"""Unreal-only3456-frame complete-event capture (144 clips, 24 frames each). No map save, inference, labels in observations.
...
OUT = Path(os.environ['BA_CAMERA_CORRIDOR_OUTPUT'])
SPEC = Path(os.environ['BA_CAMERA_CORRIDOR_SPEC'])
SCRIPT = Path(os.environ['BA_CAMERA_CORRIDOR_SCRIPT'])
PROTOCOL_SHA = os.environ['BA_CAMERA_CORRIDOR_PROTOCOL_SHA']
SPEC_SHA = os.environ['BA_CAMERA_CORRIDOR_SPEC_SHA']
SCRIPT_SHA = os.environ['BA_CAMERA_CORRIDOR_SCRIPT_SHA']
...
def component(source, fmt):
    ...
    actor.set_actor_label('BA camera corridor capture')
...
def prepare(case):
    if not frames or frames[-1]['clip_id'] != case['clip_id']:
        ...
        for obj in case['objects']:
            ...
            actor.set_actor_label('BA camera corridor '+case['clip_id']+' '+obj['name'])
            mesh_path = '/Engine/BasicShapes/'+{'cube':'Cube','cylinder':'Cylinder'}[obj['kind']]
...
def authenticate(case):
    ...
    for obj, actor in objects:
        ...
        # Target source visibility is required. Background center may be hidden
        # behind the target and is recorded without claiming it was observed.
        if obj['name'] == case['target_name']:
            assert same_actor, 'Target center native trace did not hit the target'
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==3456 and len(geometry)==3456
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==3456
            assert len({c['name'] for c in cases})==3456 and len({c['clip_id'] for c in cases})==144
            for offset in range(0, 3456, 24):
                clip = cases[offset:offset+24]
                assert len({c['clip_id'] for c in clip})==1
                assert [c['frame_in_clip'] for c in clip]==list(range(24))
                assert all(clip[i]['time_s'] < clip[i+1]['time_s'] for i in range(23))
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=3456,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=3456))
            if index==len(cases):
                fx=640/(2*math.tan(math.radians(50)))
                write(OUT/'observations/manifest.json',dict(schema='ba-camera-corridor-rgb-v1',
