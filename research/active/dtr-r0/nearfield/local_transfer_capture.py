"""Unreal-only576-frame local-transfer capture (48 clips, 12 posed frames each).
...
OUT = Path(os.environ['BA_LOCAL_TRANSFER_OUTPUT'])
SPEC = Path(os.environ['BA_LOCAL_TRANSFER_SPEC'])
SCRIPT = Path(os.environ['BA_LOCAL_TRANSFER_SCRIPT'])
PROTOCOL_SHA = os.environ['BA_LOCAL_TRANSFER_PROTOCOL_SHA']
SPEC_SHA = os.environ['BA_LOCAL_TRANSFER_SPEC_SHA']
SCRIPT_SHA = os.environ['BA_LOCAL_TRANSFER_SCRIPT_SHA']
...
def component(source, fmt):
    ...
    actor.set_actor_label('BA local transfer capture')
...
def prepare(case):
    if not frames or frames[-1]['clip_id'] != case['clip_id']:
        ...
        for obj in case['objects']:
            ...
            actor.set_actor_label('BA local transfer '+case['clip_id']+' '+obj['name'])
    ...
    try:
        ...
        if stage==4:
            ...
            if index==len(cases):
                ...
                write(OUT/'observations/manifest.json',dict(schema='ba-local-transfer-rgb-v1',
