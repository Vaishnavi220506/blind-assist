...
ToF is radial metres; Radar uses horizontal ray-hit range and causal native-relative Doppler.
...
def engine():
    ...
    from mz113_dynamic_sensors import sensors
    out=Path(os.environ['BA_MZ113_OUT']);specpath=Path(os.environ['BA_MZ113_SPEC']);spec=json.loads(specpath.read_text())
...
def main():
    ...
    for name in ('mz113_dynamic_sensors.py','mz99_angle_information_capture.py'):
    ...
    env=dict(os.environ,BA_MZ113_OUT=str(out),BA_MZ113_SPEC=str(out/'spec.json'));env['UE-LocalDataCachePath']=str(cache)
...
if os.environ.get('BA_MZ113_SPEC'):
