"""Unreal-only1152-frame complete-event capture (48 clips, 24 frames each). No map save, inference, labels in observations.
...
def finish(error=None):
    ...
    try:
        ...
        if report['status']=='PASS':
            assert len(frames)==1152 and len(geometry)==1152
    except Exception:
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['expected_map_sha256']==EXPECTED and len(cases)==1152
            assert len({c['name'] for c in cases})==1152 and len({c['clip_id'] for c in cases})==48
            for offset in range(0, 1152, 24):
        ...
        if stage==3:
            if not readiness.poll():
                write(OUT/'progress.json',dict(status='READINESS',frames=index,expected=1152,
        ...
        if stage==4:
            ...
            write(OUT/'progress.json',dict(status='RUNNING',frames=index,expected=1152))
            if index==len(cases):
