"""Fixed fresh MZ170 source, bound to a root-owned frozen-mean recipe before freeze.
...
SOURCE = WORK/'source'
SEED = 170016
PRIOR_SEEDS = (136014, 146016, 158016)
...
def _rename(value):
    if isinstance(value, str):
        return value.replace('mz136', 'mz170')
...
def check_source(spec):
    ...
    assert all(f['id'].startswith('mz170_') for f in spec['frames'])
    ...
    return dict(original, split_frames={'confirmation': 288}, source_seed=SEED,
        prior_seeds=list(PRIOR_SEEDS), prior_geometry_overlaps=[0 for _ in PRIOR_SEEDS],
