...
ToF reports hypothetical finite-footprint zone returns; Radar/IMU are unchanged MZ113.
...
def engine():
    ...
    from mz115_zonal_sensors import sensors
    out=Path(os.environ['BA_MZ115_OUT']);specpath=Path(os.environ['BA_MZ115_SPEC']);spec=json.loads(specpath.read_text())
    ...
    environment_reflectance=[]
    background=spec.get('background',dict(center_m=[10.,0.,1.5],size_m=[.1,16.,8.],texture=False))
    if background:
        background_actor=textured(background,None)
        environment_reflectance.append((background_actor.static_mesh_component,background.get('tof_reflectance_proxy',1.)))
    floor=spec.get('floor',dict(center_m=[4.,0.,-.05],size_m=[24.,20.,.1]))
    if floor:
        floor_actor=cube(floor['center_m'],floor['size_m'],.22)
        environment_reflectance.append((floor_actor.static_mesh_component,floor.get('tof_reflectance_proxy',1.)))
...
def main():
    ...
    for name in ('mz115_zonal_sensors.py','mz115_zonal_tof.py','mz113_dynamic_sensors.py','mz99_angle_information_capture.py'):
...
if os.environ.get('BA_MZ115_SPEC'):
