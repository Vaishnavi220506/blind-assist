"""Source admission and unchanged single-return observations for one fixed-B data-coverage comparison.
...
def materialize(out):
    ...
    evaluator/train-labels.json, evaluator/dev-labels.json, evaluator/evaluation-labels.json, and observation-seal.json.
    """
    from run_data_coverage import verify, seal, read, write, sha, T
    ...
    out = Path(out)
    p = verify(out)
    assert p['frames'] == 3456 and p['clips'] == 144 and p['frames_per_clip'] == 24
    assert p['dt_s'] == .2 and p['strong_threshold'] == T
    files = ['observations.npz', 'identities.json', 'baseline.json', 'private-lineage.json',
             'source-admission.json', 'evaluator/metadata.json', 'evaluator/train-labels.json',
             'evaluator/dev-labels.json', 'evaluator/evaluation-labels.json']
    ...
    assert receipt['script_sha256'] == launch['capture_script_sha256'] == p['code_hashes']['data_coverage_capture.py']
    assert launch['launcher_sha256'] == p['code_hashes']['launch_data_coverage.py']
    ...
    assert Counter(c['split'] for c in cases) == dict(train=1728, dev=576, evaluation=1152)
    from data_coverage_spec import check_spec
    check_spec(spec, [])  # The root freeze separately checks all prior source cohorts.
    groups = Counter(c['base_group_id'] for c in cases)
    clips = Counter(c['clip_id'] for c in cases)
    assert len(groups) == 48 and set(groups.values()) == {72}
    assert len(clips) == 144 and set(clips.values()) == {24}
    ...
    labels = {split: [] for split in ('train', 'dev', 'evaluation')}
    boxes = boxes45()
    for i, (case, rgb, geo) in enumerate(zip(cases, manifest, geometry)):
        ...
        if i % 384 == 0:
            print('DATA_COVERAGE_MATERIALIZE', i, '/', p['frames'], flush=True)
    ...
    for split, rows in labels.items():
        write(out / ('evaluator/'+split+'-labels.json'), rows)
    ...
    print('DATA_COVERAGE_OBSERVATIONS_SEALED', p['frames'], flush=True)
