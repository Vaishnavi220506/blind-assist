"""One pooled versus registered public-zone context contrast."""
import argparse
import os
...
from mz172_spatial_return_context import loss,evaluate_context,METHOD

WORK=ROOT/'artifacts.local/work/mz172-spatial-return-context-20260916'
PREP=ROOT/'artifacts.local/work/mz171-return-witness-20260916/preparation'
GRIDPREP=WORK/'preparation'
...
ARMS=('pooled','registered');SEED=172016;EPOCHS=20;BATCH=4;LIMIT=1200.
...
def run(out):
    ...
    def within():
        if time.perf_counter()-start>=LIMIT:raise TimeoutError('MZ1721200s allocation exhausted')
    ...
    gridseal=read(GRIDPREP/'seal.json')
    for group in ('inputs','sources'):
        for p,h in gridseal[group].items():assert sha(p)==h,p
    for n,h in gridseal['outputs'].items():assert sha(GRIDPREP/n)==h,n
    with np.load(GRIDPREP/'zone-grids.npz',allow_pickle=False) as c:
        grids,visible=c['grid'],c['visible'];assert c['ids'].tolist()==ids
    assert grids.shape==(192,64,64,2) and visible.shape==(192,64,64)
    assert grids.dtype==np.float32 and visible.dtype==np.bool_
    opportunity=read(GRIDPREP/'admission.json')
    assert opportunity['status']=='PASS' and not opportunity['raw_validity_changed']
    assert opportunity['held_native_records_decoded']==opportunity['original_dev_test_records_decoded']==0
    fit_context=visible[fitix].any(-1).repeat(2,axis=1)
    assert (slot_known[:,:128]&(slot_target[:,:128]>0)&fit_context).any()
    assert (slot_known[:,:128]&(slot_target[:,:128]==0)&fit_context).any()
    sources=local_dependencies(__file__)
    for n in ('MZ172_PROTOCOL_20260916.md','test_mz172_spatial_return_context.py','mz172_return_grid.py','test_mz172_return_grid.py'):
        sources[str(CODE/n)]=sha(CODE/n)
    inputs={**oldfreeze['inputs'],**parent_freeze['inputs'],**witness_seal['inputs'],**gridseal['inputs']}
    for n in ('seal.json','zone-grids.npz','admission.json'):inputs[str(GRIDPREP/n)]=sha(GRIDPREP/n)
    for home,names in ((OLD,('completion.json','prediction-seal.json','model-seal.json','freeze.json','input-seal.json','input-audit.json','public-features.npy','public-tokens.npz','fit-targets.npz','fit-label-seal.json','fit-label-audit.json')),
        ...
        (WORK,('head-preflight.json','pre-run-review.json'))):
        for n in names:inputs[str(home/n)]=sha(home/n)
    for n in ('head-backend.json','head-preflight.json'):
        p=WORK/n;inputs[str(p)]=sha(p)
    backend=read(WORK/'head-backend.json');preflight=read(WORK/'head-preflight.json')
    assert backend['selected_device_type']=='cuda' and preflight['parameters']==24971
    assert preflight['torch']==torch.__version__ and preflight['cuda']==torch.cuda.get_device_name(0)
    write(out/'backend.json',dict(selection='MEASURED_MZ172_POOLED_REGISTERED_HEAD_CPU_CUDA',
        prior_sha256=sha(WORK/'head-backend.json'),actual_device=torch.cuda.get_device_name(0),
        torch=torch.__version__,precision='FLOAT32_TF32_DISABLED',encoder_inference=False,
        auxiliary_loss='SAME_KNOWN_SLOT_BCE_BOTH_ARMS;REGISTERED_OR_UNIFORM_PUBLIC_ZONE_CONTEXT',prior=backend))
    for row in rows:
    ...
    write(out/'freeze.json',dict(method=METHOD,sources=sources,inputs=inputs,metadata=metadata,
        ...
        predictor_input_keys=['image','context','tokens','valid','zone_grid','zone_visible']))
    ...
    def batch(ii):
        return dict(image=torch.from_numpy(np.asarray(features[ii],np.float32)).cuda(),
            ...
            zone_grid=torch.from_numpy(grids[ii]).cuda(),zone_visible=torch.from_numpy(visible[ii]).cuda())
    predictions={};initial={};final={};timings={}
    for arm in ARMS:
        seed();model=SpatialReturnTaskNet(arm).cuda();initial[arm]=state_hash(model)
        assert sum(p.numel() for p in model.parameters())==24971
        ...
        raw_logits=np.empty_like(returned);context_logits=np.empty_like(returned)
        attention=np.empty((192,128,64),np.float32);context_valid=np.empty((192,128),bool)
        with torch.no_grad():
            for i,row in enumerate(rows):
                output=model(batch([i]));maps[i]=output['pixel'][0].cpu().numpy()
                returned[i]=output['token'][0].cpu().numpy()
                raw_logits[i]=output['token_raw'][0].cpu().numpy();context_logits[i]=output['token_context'][0].cpu().numpy()
                attention[i]=output['attention'][0].cpu().numpy();context_valid[i]=output['context_valid'][0].cpu().numpy()
                expected_context=visible[i].any(-1).repeat(2)&valid[i,:128]
                assert np.array_equal(context_valid[i],expected_context)
                assert np.allclose(attention[i].sum(-1),expected_context.astype(float),atol=1e-6)
                assert np.all(attention[i][~visible[i].repeat(2,axis=0)]==0)
                assert np.array_equal(returned[i,128:],raw_logits[i,128:])
                assert np.array_equal(returned[i,:128][~expected_context],raw_logits[i,:128][~expected_context])
                ...
                records.append(dict(id=row['id'],frame_logit=f,pixel_max=p,token_max=t,
                    ...
                    raw_tof_max=float(raw_logits[i,:128].max()),raw_radar_max=float(raw_logits[i,128:].max()),
                    contextual_tof_max=float(context_logits[i,:128].max()),
                    contextual_valid_slots=int(context_valid[i].sum()),
        ...
        for name,arr in (('raw-token-logits',raw_logits),('context-token-logits',context_logits),('attention',attention),('context-valid',context_valid)):
            np.save(out/(arm+'-'+name+'.npy'),arr)
        ...
        del model,optimizer,maps,output,value,returned,raw_logits,context_logits,attention,context_valid;release()
    assert initial['pooled']==initial['registered']
    write(out/'model-seal.json',dict(initial_states=initial,models=final,head_parameters=24971,
    ...
    write(out/'prediction-seal.json',dict(authority='ALL192_PIXEL_TOKEN_MAPS_AND_FLAGS_PER_ARM_BEFORE_HELD_NATIVE',
        outputs={a+'-'+n:sha(out/(a+'-'+n)) for a in ARMS for n in ('pixel-logits.npy','token-logits.npy','predictions.json','raw-token-logits.npy','context-token-logits.npy','attention.npy','context-valid.npy')},
    ...
    result=evaluate_context(rows,es,labels,maps,predictions,baseline,spec,metadata,logits,all_targets,all_known)
    result['summary']['context_branches']={}
    for part in ('fit','heldout'):
        ii=[i for i,r in enumerate(rows) if metadata[r['id']]['partition']==part]
        result['summary']['context_branches'][part]={}
        for arm in ARMS:
            raw=np.load(out/(arm+'-raw-token-logits.npy'))[ii];ctx=np.load(out/(arm+'-context-token-logits.npy'))[ii]
            rp=raw.max(1)>=0;cp=ctx.max(1)>=0
            result['summary']['context_branches'][part][arm]=dict(raw_only=int((rp&~cp).sum()),context_only=int((cp&~rp).sum()),
                both=int((rp&cp).sum()),neither=int((~rp&~cp).sum()),radar_positive_frames=int((raw[:,128:].max(1)>=0).sum()))
